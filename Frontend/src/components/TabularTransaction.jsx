...
function TabularTransaction({ idx, isAddVisible, didReset }) {
  ...
  return (
    <div className='flex flex-col'>
      ...
      <div className='relative -mt-[5px] border border-gray-400'>
        ...
        <section className={`transition-all ease-in-out duration-300 ${isCollapsed ? "max-h-0 collapse opacity-0" : 'max-h-[1280px] md:max-h-[620px] opacity-100'}`}>
...

          {
            activeTab === 'systematic' ?
              <SystematicForm index={idx} /> :
              activeTab === 'purchRedemp' ?
                <PurchRedempForm index={idx} /> :
                <SwitchForm index={idx} />
          }</section>
      </div>
    </div>
  )

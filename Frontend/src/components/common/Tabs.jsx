      ...
      <ul className="flex">{
        tabs.map((tab) => (
          <li className="text-sm md:text-base py-1" key={tab.id}>
            <button type='button' disabled={isCollapsed} onClick={() => onTabChange(tab.id)} className={`relative px-3 py-1 border border-b-px z-[3] ${activeTab === tab.id ? 'border-gray-400 border-b-primary-white text-light-blue' : 'border-transparent text-gray-400'}`} >{tab.name}</button>
          </li>
